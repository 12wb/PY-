...
def receiveMessage():
    # 建立Socket连接
    serverSock=socket.socket(socket.AF_INET,socket.SOCK_STREAM)  # 创建套接字
    serverSock.bind((local,port))  # 把套接字绑定到ip地址
    ...
    while True:
        ...
        while True:
            ...
            if not cientMsg:
                ...
                chatText.insert(tkinter.END, '比尔盖茨 ' + theTime +' ：\n')
...
def sendMessage():
    ...
    # 格式化当前的时间
    theTime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    chatText.insert(tkinter.END, '毕志刚 ' + theTime +' ：\n')
...
# 显示消息Text右边的滚动条
chatTextScrollBar = tkinter.Scrollbar(frame[0])  #0
...
inputTextScrollBar['command'] = inputText.yview
