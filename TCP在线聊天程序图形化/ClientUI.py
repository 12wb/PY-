...
class ClientUI():
    ...
    def __init__(self):
        ...
        # 关于按钮
        self.aboutButton = tkinter.Button(self.frame[3], text=' 关于', width=10, command=self.Command1_Cmd)
        self.aboutButton.pack(expand=1, side=tkinter.RIGHT, padx=15, pady=8)
    ...
    def Command1_Cmd(self, event=None):
        messagebox.showinfo(title='关于', message='client' + "\n" + '毕志刚  ' + "\n" + '人工智能3182班')
            ...
            try:
                if self.flag == True:
                    ...
                    if self.serverMsg == 'Y':
                        ...
                        self.chatText.insert(tkinter.END, '毕志刚 ' + theTime +' ：\n')
                        self.chatText.insert(tkinter.END, '  ' + self.serverMsg)
                else:
                    break
            except EOFError as msg:
                raise msg  # 执行了raise语句,raise之后的语句不在执行，加入了try,except,则except里的语句会被执行
    ...
    def sendMessage(self):
        # 得到用户在Text中输入的消息
        message = self.inputText.get('10',tkinter.END)
        # 格式化当前的时间
        theTime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
...
def main():
    client = ClientUI()  # 调用类
    client.startNewThread() # 调用  类里面 启动线程接收客户端的消息
    client.root.mainloop() # 刷新显示  类里面 构造函数 的界面
